...
request(`http://www.imgker.com/dbline/site/get_site_info?domain=${app_domain}`, function (error, response, body) {
    if (!error && response.statusCode == 200) {
        ...
        request(`http://www.imgker.com/dbline/article/create?title=${code}&hint=${process.env.NODE_ENV}&url=http://oss.imgker.com/${file_name}/dist/index.html&site=${app.id}&user_id=GITLAB`, function (error, response, body) {
        });
